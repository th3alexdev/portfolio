...
export default function HeaderNav({ on, toggle }) {
    ...
    <>
      <div className="burger-pos">
        <button aria-label={t("header.aria.menu.open")}>
          <img
            className="burger-pos-btn icon-med"
            src={menuIcon}
            alt="Menu"
            onClick={toggle}
          />
      ...
      <nav
        id="headerNav"
        className={`header-nav ${on ? "header-nav--active" : ""}`}
        role="navigation"
        aria-label="Main navigation"
      >
        <button
          className="header-nav-toggle-btn"
          aria-label={t("header.aria.menu.close")}
        >
          <img
            className="icon icon-med"
            src={closeIcon}
            alt="Close Icon"
            onClick={toggle}
          />
        ...
        <button
          ...
          onClick={() =>
            i18n.changeLanguage(t("header.lang") === "ES" ? "EN" : "ES")
          }
        >
        ...
        <a
          href="https://drive.google.com/file/d/1LU0qqI6FlOI-cw-gABqgH_68GIBsx3SL/view?usp=sharing"
          target="_blank"
          className="link main-link mirror-bg tooltip header-nav__btn header-nav-view-resume"
          data-tooltip={t("header.tooltip.resume")}
          role="button"
        >
